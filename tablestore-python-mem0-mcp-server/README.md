...
| TOOL_USER_ID_FIELD_DESCRIPTION                                 |                             _否_                              | user_id字段的描述文字，用于引导智能体提取user_id |       参考settings.py       |
...
其中 `{user_id}` 为可选参数，以下两种 URL 均为合法格式：
- `http://localhost:8765/mcp/{client_name}/sse/{user_id}` — 指定默认的 user_id
- `http://localhost:8765/mcp/{client_name}/sse` — 不指定 user_id，由工具参数或智能体自动提取

> **user_id 参数说明**：在 sse 模式下，每个 MCP 工具都支持通过 `user_id` 参数直接指定操作的用户。智能体会尝试从用户输入或对话上下文中提取 `user_id`，如果未提取到，则回退到 URL 路径中的 `{user_id}` 值，如果两者均未提取到，则报错。
>
> 示例：当用户说"查询 alice 的记忆"时，智能体会自动将 `user_id=alice` 传给工具；当用户未指定用户时，工具会使用 URL 中配置的 user_id。
...
> **user_id 参数说明**：在 stdio 模式下，每个 MCP 工具同样支持通过 `user_id` 参数直接指定操作的用户。如果智能体未从用户输入中提取到 `user_id`，则回退到环境变量 `MCP_STDIO_USER_ID` 配置的默认值。
...
每个 MCP 工具都支持通过 `user_id` 参数直接指定操作的目标用户，智能体会尝试从用户的输入或对话上下文中自动提取 `user_id` 的值。

**使用示例**：

- 用户说："帮我记住 alice 喜欢冰激凌" → 智能体自动提取 `user_id=alice`，调用 `add_memories(text="喜欢冰激凌", user_id="alice")`
- 用户说："查询 bob 的所有记忆" → 智能体自动提取 `user_id=bob`，调用 `list_memories(user_id="bob")`
- 用户说："搜索我的记忆" → 未指定 user_id，工具回退到上下文中的默认 user_id

以下示例展示了智能体从用户输入中自动提取 `user_id` 并调用工具的效果：

<img src="./docs/img/基于user_id查询.png" alt="基于user_id查询" width="500">

**user_id 优先级**：工具入参 `user_id` > contextvars 上下文变量（sse 模式下来自 URL 路径，stdio 模式下来自 `MCP_STDIO_USER_ID` 环境变量）

**自定义 user_id 字段描述**：

如果需要自定义 `user_id` 字段的描述文字（例如更换语言或调整引导策略），可以通过环境变量进行配置：

```shell
  export TOOL_USER_ID_FIELD_DESCRIPTION="要操作的用户ID，请从用户输入或对话上下文中提取user_id，如果未提供则使用上下文默认值"
```
